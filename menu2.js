	...
	class Box {
		...
		open() {
			return new Promise((resolve, reject) => {
				this.DOM.bg.style.transformOrigin = this.pos % 2 === 0 ? '50% 100%' : '100% 0%';
				...
				anime({
					...
					complete: () => {
						if ( this.DOM.img && this.DOM.cover ) {
							...
							this.DOM.cover.style.transformOrigin = '0% 100%';
		...
		close() {
			return new Promise((resolve, reject) => {
				const animateBoxFn = () => {
					this.DOM.bg.style.transformOrigin = this.pos % 2 === 0 ? '50% 0%' : '0% 100%';
